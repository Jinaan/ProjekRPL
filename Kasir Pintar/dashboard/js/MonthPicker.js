...
(function($, window, document, Date) {
    ...
    function _applyButtonHint(_button, _hintText) {
        var _speed = 125,
            _currentLabel, _startTimeout, _labelElem = $();
        ...
        function _fadeOutLabel() {
            _startTimeout = null;
            _labelElem = $('span', _button).animate({
                opacity: 0.45
            }, _speed, _fadeInHint);
        }
        ...
        function _fadeInLabel() {
            _labelElem.text(_currentLabel).animate({
                opacity: 1
            }, _speed);
        }
    ...
    $.widget("KidSysco.MonthPicker", {
        ...
        _createButton: function() {
            ...
            if ($.isFunction(_btnOpt)) {
                var _params = $.extend(true, {
                    i18n: $.extend(true, {}, $.MonthPicker.i18n)
                }, this.options);
        ...
        _createValidationMessage: function() {
            var _errMsg = this.options.ValidationErrorMessage,
                _elem = this.element;
        ...
        _position: _hasPosition ?
            function($menu) {
                ...
                return $menu.position($.extend({ of: this.element
                }, _posOpts));
            } : function($menu) {
        ...
        _showMonths: function() {
            ...
            var me = this,
                _onMonthClick = $proxy(me._onMonthClick, me);
        ...
        _decorateButtons: function() {
            var _curYear = this._getPickerYear(),
                _todaysMonth = _toMonth(new Date),
                _minDate = this._MinMonth,
                _maxDate = this._MaxMonth;
            ...
            for (var i = 0; i < 12; i++) {
                ...
                var _month = (_curYear * 12) + i,
                    _isBetween = _between(_month, _minDate, _maxDate);
