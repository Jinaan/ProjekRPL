    ...
    <div class="wrapper">
        ...
        <div class="main-panel-v2">
            ...
            <div class="padd-main bg-main" id="get_checkbox">
                <div class="d-flex">
                    <div style="flex: 1">
                        <!-- <span class="font-18 semi-bold font-black">Kasir</span><br> -->
                    </div>
                    <div>
                        <a href="../account/database?guide=true"
                            class="font-14 medium font-green" id="goTutor" style="display: none;">Tutorial</a>
                    </div>
                </div>
                <div class="card padd-card margin-top-20">
                    <div class="row">
                        <div class="col-md-9">
                            <span class="font-20 semi-bold font-black">KASIR</span>
                            &nbsp;&nbsp;


                            <button class="btn button-merah" type="button" id="btnHapus"
                                style="display: none";>
                                <!-- style="display: none; "> -->
                        ...
                        <div class="col-md-3 mt-mobile">
                            <div class="input-group">
                                <!-- <form action="" method="post"> -->
                                    <input type="search" id="myInputTextField" placeholder="Masukkan Kode Barang"
                    ...
                    <div class="row">
                        <div class="col-md-9">
                            <span class="font-18 semi-bold font-black">Total Harga : Rp <span id="TotalHargaTop">0</span></span>
                        </div>
                        <div class="col-md-3 mt-mobile">
                            <!-- bayar button -->
                            <button class="btn button-hijau" type="button" id="btnBayar" disabled
                             >Bayar</button>
                            &nbsp;
                        </div>
                    </div>
                        ...
                        <table class="table datatable mdl-data-table dataTable" cellspacing="0" nwidth="100%"
                            ...
                            <thead class="tb-border">
                                <tr>
                                    ...
                                    <td class="tb-head">Kategori</td>
                                    <td class="tb-head">Harga Jual (Rp)</td>
                                    <td class="tb-head">Jumlah</td>
                                    <td class="tb-head" style="width: 250px;">Total (Rp)</td>
                                </tr>
                            </thead>
                            <tbody class="font-14" id="BodyTabelKasir">
                            </tbody>
                            <tbody class="font-14">
                            </tbody>
                        </table>
...
<script>
    function checkBarang(){
        // console.log($("#BodyTabelKasir").children().length);
        if($("#BodyTabelKasir").children().length > 0){
...
<script>
    // BodyTabelKasir
    var bodyTabel = document.getElementById('BodyTabelKasir');
    ...
    var observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            ...
            var selectednow = $(".selected").length;
            if(selectednow <= 0){
    ...
    observer.observe(bodyTabel, config);
...
<script>
    $(document).ready(function () {
        $("#btnBayar").click(function () {
            ...
            var kodeBarang = [];
            var jumlahBarang = [];
            var tbody = document.getElementById("BodyTabelKasir");
            for(var i = 0; i < tbody.children.length; i++){
                var row = tbody.children[i];
                var kode = row.cells[1].innerHTML;
                var jumlah = row.cells[5].querySelector("input").value;
                var harga = row.cells[4].innerHTML;
                kodeBarang.push(kode);
            ...
            xhr.open("POST", "Action/kasir/bayar.php", true);
...
<script>
    ...
    function hitungTotal(idRow) {
        idRow = "row" + idRow;
        var hargaJual = document.getElementById(idRow);
        var kode = document.getElementById(idRow).cells[1].innerHTML;
        var stok = 0;
        for (var i = 0; i < dataBarang.length; i++) {
            if (dataBarang[i].kode_barang == kode) {
                stok = dataBarang[i].stok;
            }
        }

        var jumlah = document.getElementById(idRow).cells[5].querySelector('input').value;
        jumlah = parseInt(jumlah);
        if (jumlah <= 0) {
            jumlah = 1;
            document.getElementById(idRow).cells[5].querySelector('input').setAttribute('value', jumlah);
        } else if (jumlah > stok) {
            jumlah = stok;
        }
        ...
        hargaJual = hargaJual.cells[4].innerHTML;
        hargaJual = hargaJual.replace(',', '');
        // update attribute value
        document.getElementById(idRow).cells[5].querySelector('input').setAttribute('value', jumlah);
        document.getElementById(idRow).cells[5].querySelector('input').value = jumlah;
        // console.log(jumlah);
        var total = hargaJual * jumlah;
        // add ',' every 3 digit
        total = total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        document.getElementById(idRow).cells[6].innerHTML = total;
        totalDisplay();
    }
    

    function totalDisplay () {
        var totalHarga = document.getElementById('TotalHargaTop');
        // BodyTabelKasir
        var bodyTabelKasir = document.getElementById('BodyTabelKasir');
        var total = 0;
        var row = bodyTabelKasir.rows.length;
        if (row == 0) {
            totalHarga.innerHTML = 0;
            return;
        }
        for (var i = 0; i < bodyTabelKasir.rows.length; i++) {
            var totalHargaBaris = bodyTabelKasir.rows[i].cells[6].innerHTML;
            while (totalHargaBaris.includes(',')) {
                totalHargaBaris = totalHargaBaris.replace(',', '');
            }
            total += parseInt(totalHargaBaris);
        }
        total = total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
        // console.log(total);
    ...
    $('#myInputTextField').keypress(function (e) {
        if (e.which == 13) {
            var searchValue = $('#myInputTextField').val();
            var xhr = new XMLHttpRequest();
            var urlNow = window.location.href;
            var tail = "Action/kasir/getBarang.php";
            var url_search = urlNow.replace("Kasir.php", tail);
            var tbody = document.getElementById('BodyTabelKasir');
            var postData = "search=" + searchValue;
            xhr.open('POST', url_search, true);
            ...
            xhr.onload = function () {
                if (this.status == 200) {
                    ...
                    var output = '';
                        output += '<tr id="row' + rowInitNumber + '">';
                        output += '<td class=" select-checkbox"></td>';
                        output += '<td>' + data.kode_barang + '</td>';
                        output += '<td>' + data.nama_barang + '</td>';
                        output += '<td>' + data.kategori + '</td>';
                        output += '<td>' + data.hargaJual + '</td>';
                        output += '<td><input type="number" class="form-control" value="1" style="width: 100px;" onchange="hitungTotal(' + rowInitNumber + ')"></td>';
                        output += '<td>' + data.hargaJual + '</td>';
                        output += '</tr>';
                        rowInitNumber++;
                    // console.log(output);
    ...
    $(document).ready(function () {
        $('#btnHapus').click(function (e) {
            if (selections.length > 1000) {
                $('#hapusBarangReset').modal('show');
            } else {
                $('#detailBulkRemove').html(null);
                $('#detailBulkRemove').hide();
                $('#yakin').html('Saya telah yakin untuk menghapus produk');
                $('#hapusBarang').modal('show');
            }
        });
        var $remove = $('#btnHapus');
        var $removeBarang = $('#hapus_barang');
        var selections = [];
        
        $('#allBarang').click(function () {
